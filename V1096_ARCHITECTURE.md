...
## Runtime
`core/legacy/*.js` remains the verified compatibility source-of-truth for this release. The migration is facade-first: new code gets one canonical public contract while legacy APIs remain available until dependency usage is proven safe to remove.
...
- `core/styles/ui-architecture.css` → canonical V1097 layer
...
Do not delete legacy economy functions in V1097. New modules must use `ZIVOZONE.Economy`. Legacy calls are removed only after a dependency map confirms zero live references.

## CSS migration rule
The news rail base styles are now owned by `core/styles/ui-architecture.css`; the legacy bundle keeps the remaining historical styles to avoid visual regressions. Future CSS extraction should be selector-verified and incremental.
