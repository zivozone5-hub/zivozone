...
(() => {
  ...
  function renderChallenges(){
    ...
    const horror=C.horror||C.get?.('horror');
    const horrorCard=horror?`<article class="challenge-card special challenge-card-v47 horror-card-v47 horror-featured" data-challenge-card="horror">
      <div class="horror-featured-glow"></div>
      <div class="challenge-card-top"><span class="challenge-icon-v47 horror-eye" aria-hidden="true">👁</span><span class="card-tag danger">∞ · SPECIAL</span></div>
      <div class="challenge-card-body"><span class="eyebrow horror-eyebrow">ZIVOZONE DARK ROOM</span><h3>${esc(loc(horror.title)||'الغرفة المظلمة')}</h3><p>${esc(loc(horror.desc)||'تجربة نفسية مستقلة. لا تدخلها وحدك.')}</p>
        <div class="challenge-card-meta horror-meta"><span>🌑 تجربة مستقلة</span><span>🎧 صوت محيطي</span><span>⚠️ رعب نفسي</span></div>
      </div>
    ...
    const normal=list.map((x,i)=>`<article class="challenge-card challenge-card-v47" data-challenge-card="${esc(x.id)}" style="--card-index:${i}">
      ...
      <div class="challenge-card-body"><h3>${esc(x.title)}</h3><p>${esc(x.desc)}</p><div class="challenge-card-meta"><span>⏱ 10s</span>${x.xp?`<span>⚡ ${x.xp} XP</span>`:''}</div></div>
      <button class="btn btn-primary challenge-start-v47" data-challenge="${esc(x.id)}">${esc(t('start'))} <span aria-hidden="true">←</span></button>
    </article>`).join('');
    container.innerHTML=horrorCard+normal;
  }
  ...
  document.addEventListener('keydown',e=>{if(e.key==='Escape'&&game.id){quitGame()}},true);
  ...
  function bindSportsFilters(){
    const buttons=$$('.sports-filter');
    if(!buttons.length)return;
    buttons.forEach(b=>{b.onclick=()=>{
      buttons.forEach(x=>x.classList.toggle('active',x===b));
      const f=b.dataset.sportFilter;
      ['news-list','jordan-news-list','sports-list','fixture-list'].forEach(id=>{const el=$('#'+id);if(el)el.hidden=(f==='jordan'&&id!=='jordan-news-list')||(f==='matches'&&id!=='fixture-list')||(f==='football'&&id==='jordan-news-list');});
      if(f==='all') ['news-list','jordan-news-list','sports-list','fixture-list'].forEach(id=>{$('#'+id)?.removeAttribute('hidden')});
    }});
  }
  function bind(){bindSportsFilters();const audioBtn=$('#audio-toggle');if(audioBtn){audioBtn.textContent=S().isEnabled?.()?'🔊':'🔇';audioBtn.onclick=async()=>{await S().unlock?.();S().toggle?.();audioBtn.textContent=S().isEnabled?.()?'🔊':'🔇'}}$('#language-select').value=lang();$('#language-select').onchange=async e=>{I.set(e.target.value);await A.setLanguage(e.target.value);applyLanguage();toast(t('updateDone'),'success')};$('#login-btn').onclick=()=>A.isLoggedIn()?location.hash='#profile':authModal();$('#ai-form').onsubmit=async e=>{e.preventDefault();const input=$('#ai-input'),v=input.value.trim();if(!v)return;const box=$('#ai-messages');const u=document.createElement('div');u.className='ai-message user';u.textContent=v;box.append(u);input.value='';const b=document.createElement('div');b.className='ai-message bot';b.textContent='…';box.append(b);b.textContent=await askAI(v);box.scrollTop=box.scrollHeight}}
