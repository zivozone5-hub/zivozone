...
function challengeVisual(x){const id=String(x?.id||'').toLowerCase();if(id==='horror')return 'assets/challenges/horror.svg';if(id.includes('football'))return 'assets/challenges/football.svg';if(id.includes('math'))return 'assets/challenges/math.svg';if(id.includes('memory'))return 'assets/challenges/memory.svg';if(id.includes('science'))return 'assets/challenges/science.svg';if(id.includes('strategy'))return 'assets/challenges/strategy.svg';if(id.includes('focus'))return 'assets/challenges/focus.svg';if(id.includes('pattern'))return 'assets/challenges/pattern.svg';return 'assets/challenges/logic.svg'}
  function challengeDescription(x){return loc(x?.desc??x?.description??x?.subtitle??'تحدٍ ذكي ومتدرج في الصعوبة.')}
  function renderChallenges(){const today=new Date().toISOString().slice(0,10),done=localStorage.getItem('zivo_daily_'+today);const raw=C.getAll();const seenIds=new Set();const list=raw.filter(x=>x&&x.id&&!seenIds.has(x.id)&&seenIds.add(x.id));$('#challenge-list').innerHTML=list.map(x=>{const icon=x.icon||'🧠',title=loc(x.title||x.name||'تحدٍ'),desc=challengeDescription(x);return `<article class="challenge-card"><div class="challenge-image-wrap"><img class="challenge-image" src="${challengeVisual(x)}" alt="${esc(title)}" loading="lazy"><span class="challenge-icon">${esc(icon)}</span></div><div class="challenge-card-copy"><span class="card-tag">${t('questions10')}</span><h3>${esc(title)}</h3><p>${esc(desc)}</p><small class="challenge-meta-line">⏱ 10 ${lang()==='ar'?'ثوانٍ لكل سؤال':'seconds / question'} · XP ${Number(x.xp)||0}</small></div><button class="btn btn-primary" data-challenge="${esc(x.id)}" ${done&&x.id==='daily'?'disabled':''}>${done&&x.id==='daily'?t('done'):t('start')}</button></article>`}).join('')+`<article class="challenge-card special"><div class="challenge-image-wrap"><img class="challenge-image" src="assets/challenges/horror.svg" alt="${esc(loc(C.horror.title))}" loading="lazy"><span class="challenge-icon">👁️</span></div><div class="challenge-card-copy"><span class="card-tag danger">∞</span><h3>${esc(loc(C.horror.title))}</h3><p>${esc(challengeDescription(C.horror))}</p><small class="challenge-meta-line">∞ ${lang()==='ar'?'تجربة متصاعدة':'escalating experience'}</small></div><button class="btn btn-primary danger-btn" data-challenge="horror">${t('enter')}</button></article>`}
  async function sports(){renderNewsSources();window.ZIVOZONE_NEWS?.load?.($('#news-list'),lang());window.ZIVOZONE_NEWS?.loadJordan?.($('#jordan-news-list'),lang());window.ZIVOZONE_FIXTURES?.loadToday?.($('#today-fixture-list'),lang());window.ZIVOZONE_FIXTURES?.load?.($('#fixture-list'),lang());const box=$('#sports-list');if(!box)return;box.innerHTML=`<article class="sports-card loading-card"><div class="spinner"></div><h3>${esc(t('sportsTitle'))}</h3><p>${esc(t('loader'))}</p></article>`;const teamIds=['133604','133602','133738','133739'];let events=[];try{const data=await Promise.all(teamIds.map(id=>fetch(`https://www.thesportsdb.com/api/v1/json/123/eventsnext.php?id=${id}`).then(r=>r.ok?r.json():null).catch(()=>null)));data.forEach(d=>{if(d?.events)events.push(...d.events)})}catch(e){}const seen=new Set();events=events.filter(e=>{const k=e.idEvent||`${e.strEvent}-${e.dateEvent}`;if(seen.has(k))return false;seen.add(k);return true}).slice(0,8);if(!events.length){box.innerHTML=`<article class="sports-card"><div class="icon">📡</div><h3>${esc(t('noData'))}</h3><p>${esc(t('newsUnavailable'))}</p></article>`;return}box.innerHTML=events.map(e=>`<article class="sports-card"><div class="match-icon">⚽</div><span class="card-tag">${esc(e.strLeague||e.strSport||'Sport')}</span><h3>${esc(e.strHomeTeam||'Home')} <span class="versus">VS</span> ${esc(e.strAwayTeam||'Away')}</h3><p>${esc(e.dateEvent||'')} ${esc(e.strTime||'')}</p><a class="btn btn-ghost" href="https://www.thesportsdb.com/" target="_blank" rel="noopener noreferrer">${t('open')}</a></article>`).join('')}
