...
(() => {
  ...
  async function fetchFeed(f){const url=`https://site.api.espn.com/apis/site/v2/sports/${f.sport}/${f.league}/news`;const d=await (window.ZIVOZONE_RUNTIME?.fetchJSON?.(url,{timeout:9000,fallback:null})||null);if(!d)throw new Error('news');return(d.articles||[]).slice(0,5).map(a=>({headline:a.headline||a.description||'Sports news',description:a.description||'',url:a.links?.web?.href||'https://www.espn.com/',image:a.images?.[0]?.url||'',published:a.published||'',feed:f}))}
