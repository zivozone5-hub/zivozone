   ...
   feed.innerHTML=docs.map(r=>`
     <article class="z1084-reel-card" data-reel-id="${zrEsc(r.id)}">
       <div class="z1084-reel-media">
         <video src="${zrEsc(r.videoUrl)}" playsinline muted loop preload="metadata" controls></video>
...
async function publishReel(form){
 ...
 const d=zrDb(),st=zrStorage();
 if(!d||!st)throw Error('Firebase Storage/Firestore غير جاهز');
 const file=form.querySelector('#zr-video').files[0];
 ...
 if(!file)throw Error('اختر فيديو أولًا.');
 if(!title)throw Error('اكتب عنوان الـReel.');
 if(!/^video\/(mp4|webm|quicktime)$/i.test(file.type))throw Error('صيغة الفيديو المسموحة: MP4 أو WebM أو MOV.');
 if(file.size>50*1024*1024)throw Error('الحد الأقصى للفيديو 50MB في الإصدار الأول.');
 const safe=title.replace(/[^\p{L}\p{N}_-]+/gu,'-').slice(0,60)||'reel';
 const path=`reels/${Date.now()}-${safe}-${Math.random().toString(36).slice(2,8)}.${file.type==='video/webm'?'webm':file.type==='video/quicktime'?'mov':'mp4'}`;
 const snap=await st.ref(path).put(file,{contentType:file.type,customMetadata:{uploadedBy:ZR_ADMIN}});
 const url=await snap.ref.getDownloadURL();
 await d.collection('reels').add({
   title,category,description,challengeId:challengeId||null,
   videoUrl:url,storagePath:path,published:true,
   likes:0,publishedAt:firebase.firestore.FieldValue.serverTimestamp(),
   createdBy:ZR_ADMIN
 });
 form.reset();
 await loadReels();
...
function adminReelsPanel(){
 ...
 return `<section class="z1084-admin-reels">
   <div class="z103-panel-head"><div><h3>🎬 إدارة ZIVO REELS</h3><small>الإصدار الأول: النشر من ADMIN فقط</small></div></div>
   <form id="zr-upload-form" class="z1084-upload-form">
     ...
     <label class="z1084-file-label">الفيديو <input id="zr-video" type="file" accept="video/mp4,video/webm,video/quicktime" required></label>
