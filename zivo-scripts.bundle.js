...
(() => {
  ...
  let game={id:null,questions:[],index:0,score:0,pressureScore:0,streak:0,bestStreak:0,answers:[],guest:true,locked:false,horrorSignupShown:false,horrorUsed:[],timedOut:0,questionStartedAt:0,lastSurpriseAt:-99};
  ...
  function nextQuestion(delay=500){clearQuestionTimer();setTimeout(()=>{game.index++;game.locked=false;if(game.id==='horror'&&game.index%10===0){S().checkpoint?.();horrorCheckpoint();return}if(game.id==='horror'&&game.guest&&game.index===5&&!game.horrorSignupShown){game.horrorSignupShown=true;horrorSignupGate();return}renderQuestion()},delay)}
  ...
  function horrorSignupGate(){S().whisper?.();openModal(`<div class="horror-checkpoint signup-horror"><span class="eyebrow">${t('horrorThreshold')}</span><h2>${t('horrorSignupTitle')}</h2><p>${t('horrorSignupText')}</p><p class="horror-warning">${t('horrorWarning')}</p><div class="modal-actions"><button class="btn btn-primary" id="horror-signup">${t('createNow')}</button><button class="btn btn-ghost" data-action="quit-game">${t('exit')}</button></div></div>`,'horror-modal phase-2 horror-gate-locked');const bg=document.querySelector('#modal-root .modal-backdrop');if(bg)bg.onclick=null;$('#horror-signup').onclick=()=>authModal(()=>{closeModal();game.guest=false;renderQuestion()});$('#modal-root [data-action="quit-game"]').onclick=quitGame}
  const HORROR_LINES={
