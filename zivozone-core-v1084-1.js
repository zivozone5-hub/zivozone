/* ZIVOZONE V1082 — SINGLE UNIFIED JS CORE
   Execution order intentionally preserved from V1081. */
...
/* ============================================================
   ZIVOZONE V1084 — REELS CONTENT ENGINE
   One Core: uses the existing Firebase Auth/Firestore/Storage.
   V1 policy: ADMIN publishes; public users read published reels.
   No duplicate economy, auth, XP or challenge engine.
============================================================ */
(()=>{
'use strict';
const ZR_ADMIN='raefalbtish@gmail.com';
const zrDb=()=>window.firebase?.firestore?.();
const zrStorage=()=>window.firebase?.storage?.();
const zrAuth=()=>window.firebase?.auth?.();
const zrOwner=()=>String(zrAuth()?.currentUser?.email||'').trim().toLowerCase()===ZR_ADMIN;
const zrEsc=v=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
const zrFmt=v=>{try{return v?.toDate? v.toDate().toLocaleDateString('ar-JO',{day:'numeric',month:'short',year:'numeric'}):new Date(v).toLocaleDateString('ar-JO',{day:'numeric',month:'short',year:'numeric'})}catch(e){return ''}};

let zrLoaded=false;
async function loadReels(){
 const feed=document.getElementById('reels-feed'),status=document.getElementById('reels-status');
 if(!feed)return;
 status.textContent='جارٍ تحميل Reels…';
 const d=zrDb();
 if(!d){status.textContent='تعذر الاتصال بمصدر المحتوى.';return}
 try{
   const snap=await d.collection('reels').where('published','==',true).limit(50).get();
   const docs=snap.docs.map(x=>({id:x.id,...x.data()})).sort((a,b)=>{
     const ta=a.publishedAt?.toMillis?a.publishedAt.toMillis():Date.parse(a.publishedAt||0)||0;
     const tb=b.publishedAt?.toMillis?b.publishedAt.toMillis():Date.parse(b.publishedAt||0)||0;
     return tb-ta;
   }).slice(0,30);
   if(!docs.length){
     feed.innerHTML='<div class="z1084-reels-empty"><b>🎬 ZIVO REELS</b><span>سيظهر أول Reel هنا عند نشره من الإدارة.</span></div>';
     status.textContent='لا توجد Reels منشورة حاليًا.';
     zrLoaded=true; return;
   }
   feed.innerHTML=docs.map(r=>`
     <article class="z1084-reel-card" data-reel-id="${zrEsc(r.id)}">
       <div class="z1084-reel-media">
         ${r.videoType==='youtube' ? `<iframe src="${zrEsc((()=>{let u=r.videoUrl,m=u.match(/youtu\.be\/([^?&]+)/i);if(m)return 'https://www.youtube.com/embed/'+m[1]+'?rel=0';m=u.match(/[?&]v=([^&]+)/i);if(m)return 'https://www.youtube.com/embed/'+m[1]+'?rel=0';m=u.match(/youtube\.com\/shorts\/([^?&]+)/i);return m?'https://www.youtube.com/embed/'+m[1]+'?rel=0':u})())}" title="${zrEsc(r.title||'ZIVOZONE Reel')}" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>` : `<video src="${zrEsc(r.videoUrl)}" playsinline muted loop preload="metadata" controls></video>`}
         <span class="z1084-reel-badge">REEL</span>
       </div>
       <div class="z1084-reel-info">
         <div class="z1084-reel-meta"><span>${zrEsc(r.category||'عام')}</span><time>${zrFmt(r.publishedAt)}</time></div>
         <h3>${zrEsc(r.title||'ZIVOZONE Reel')}</h3>
         ${r.description?`<p>${zrEsc(r.description)}</p>`:''}
         <div class="z1084-reel-actions">
           <button type="button" class="btn btn-ghost z1084-reel-like" data-reel-like="${zrEsc(r.id)}">❤️ <span>${Number(r.likes||0)}</span></button>
           <button type="button" class="btn btn-ghost z1084-reel-challenge" data-reel-challenge="${zrEsc(r.challengeId||'')}">🎯 ${r.challengeId?'التحدي':'تفاعل'}</button>
         </div>
       </div>
     </article>`).join('');
   status.textContent=`${docs.length} Reel · آخر المحتوى المنشور`;
   zrLoaded=true;
 }catch(e){
   console.warn('ZIVO Reels load:',e);
   feed.innerHTML='<div class="z1084-reels-empty"><b>تعذر تحميل Reels</b><span>تحقق من Firestore Index/Rules ثم أعد المحاولة.</span></div>';
   status.textContent='تعذر تحميل المحتوى.';
 }
}

async function toggleLike(id,button){
 const d=zrDb(),u=zrAuth()?.currentUser;
 if(!d||!u||!id)return;
 button.disabled=true;
 try{
   const ref=d.collection('reels').doc(id).collection('likes').doc(u.uid);
   const mine=await ref.get();
   const reel=d.collection('reels').doc(id);
   if(mine.exists){
     await ref.delete();
     await reel.update({likes:firebase.firestore.FieldValue.increment(-1)});
   }else{
     await ref.set({uid:u.uid,createdAt:firebase.firestore.FieldValue.serverTimestamp()});
     await reel.update({likes:firebase.firestore.FieldValue.increment(1)});
   }
   await loadReels();
 }catch(e){console.warn('Reel like:',e);alert('تعذر حفظ التفاعل الآن.')}
 finally{button.disabled=false}
}

function challenge(id){
 if(!id)return;
 const b=document.querySelector(`[data-challenge="${CSS.escape(id)}"]`);
 if(b){b.click();return}
 location.hash='#challenges';
}

async function publishReel(form){
 if(!zrOwner())throw Error('غير مصرح');
 const d=zrDb();
 if(!d)throw Error('Firebase Firestore غير جاهز');
 const url=form.querySelector('#zr-video-url').value.trim();
 const title=form.querySelector('#zr-title').value.trim();
 const category=form.querySelector('#zr-category').value;
 const description=form.querySelector('#zr-description').value.trim();
 const challengeId=form.querySelector('#zr-challenge').value.trim();
 if(!url)throw Error('ضع رابط الفيديو.');
 if(!/^https?:\/\//i.test(url))throw Error('رابط الفيديو غير صالح.');
 if(!title)throw Error('اكتب عنوان الـReel.');
 const isYouTube=/((youtube\.com\/(shorts\/|watch\?v=))|(youtu\.be\/))/i.test(url);
 const isDirect=/\.(mp4|webm|mov)(\?.*)?$/i.test(url);
 if(!isYouTube && !isDirect)throw Error('ضع رابط فيديو مباشر MP4/WebM/MOV أو رابط YouTube/Shorts.');
 await d.collection('reels').add({
   title,category,description,challengeId:challengeId||null,
   videoUrl:url,videoType:isYouTube?'youtube':'direct',
...
function adminReelsPanel(){
 if(!zrOwner())return '';
 return `<section class="z1084-admin-reels">
   <div class="z103-panel-head"><div><h3>🎬 إدارة ZIVO REELS</h3><small>الإصدار الأول: النشر من ADMIN فقط · بدون Firebase Storage</small></div></div>
   <form id="zr-upload-form" class="z1084-upload-form">
     <label>عنوان Reel<input id="zr-title" maxlength="100" required placeholder="مثال: هدف الأردن 🔥"></label>
     <label>التصنيف<select id="zr-category"><option>كرة القدم</option><option>الأردن</option><option>أخبار</option><option>رياضة</option><option>عالمي</option><option>عام</option></select></label>
     <label>التحدي المرتبط <input id="zr-challenge" maxlength="80" placeholder="اختياري: daily"></label>
     <label>وصف قصير<textarea id="zr-description" maxlength="240" rows="2" placeholder="وصف اختياري"></textarea></label>
     <label class="z1084-url-label">رابط الفيديو <input id="zr-video-url" type="url" required placeholder="https://...mp4 أو رابط YouTube/Shorts"></label>
     <div class="z1084-upload-row"><button class="btn btn-primary" type="submit">🚀 نشر Reel</button><span id="zr-upload-status" class="muted"></span></div>
     <small class="z1084-free-note">الإصدار المجاني لا يرفع ملفات الفيديو إلى Firebase؛ استخدم رابط MP4/WebM/MOV مباشر أو رابط YouTube/Shorts.</small>
   </form>
 </section>`;
}

function wireAdminReels(o){
 const form=o?.querySelector('#zr-upload-form'); if(!form||form.dataset.bound)return;
 form.dataset.bound='1';
 form.addEventListener('submit',async e=>{
   e.preventDefault();const b=form.querySelector('button[type=submit]'),s=form.querySelector('#zr-upload-status');
   b.disabled=true;s.textContent='جارٍ الرفع…';
   try{await publishReel(form);s.textContent='تم نشر Reel بنجاح.'}
   catch(err){s.textContent=err.message||'فشل النشر.'}
...
document.addEventListener('click',e=>{
 if(e.target.closest('#login-btn')){
   setTimeout(injectAdminPanel,700);
 }
 const like=e.target.closest('[data-reel-like]');
 if(like){toggleLike(like.dataset.reelLike,like);return}
 const ch=e.target.closest('[data-reel-challenge]');
 if(ch){challenge(ch.dataset.reelChallenge);return}
 if(e.target.closest('#reels-refresh'))loadReels();
});
window.addEventListener('zivozone-auth',()=>setTimeout(()=>{injectAdminPanel();if(!zrLoaded)loadReels()},500));
window.addEventListener('hashchange',()=>{if(location.hash==='#reels')loadReels()});
