...
(() => {
  ...
  function renderChallenges(){const today=new Date().toISOString().slice(0,10),done=localStorage.getItem('zivo_daily_'+today);const raw=C.getAll();const seenIds=new Set();const list=raw.filter(x=>x&&x.id&&x.id!=='horror'&&!seenIds.has(x.id)&&seenIds.add(x.id));const visual={iq:'assets/challenges/iq.svg',science:'assets/challenges/science.svg',daily:'assets/challenges/daily.svg',football:'assets/challenges/football.svg',logic:'assets/challenges/logic.svg',memory:'assets/challenges/memory.svg',math:'assets/challenges/math.svg',forensic:'assets/challenges/forensic.svg',strategy:'assets/challenges/strategy.svg',language_v22:'assets/challenges/language.svg',logic_extreme:'assets/challenges/logic.svg',memory_focus:'assets/challenges/memory.svg',football_intelligence:'assets/challenges/football.svg',probability_v22:'assets/challenges/math.svg'};const cardVisual=x=>visual[x.id]?`<img class="challenge-visual" src="${visual[x.id]}" alt="${esc(loc(x.title))}" loading="lazy">`:`<div class="challenge-visual challenge-visual-fallback" aria-hidden="true">${esc(x.icon||'🎯')}</div>`;$('#challenge-list').innerHTML=list.map(x=>`<article class="challenge-card" data-challenge="${esc(x.id)}">${cardVisual(x)}<div class="challenge-copy"><span class="card-tag">${x.id==='forensic'?'20 سؤال':'10 أسئلة'} · 30s</span><h3>${esc(loc(x.title))}</h3><p>${esc(loc(x.desc||'تحدٍ تفاعلي مصمم لقياس مهارة محددة.'))}</p></div><button class="btn btn-primary" data-challenge="${esc(x.id)}" ${done&&x.id==='daily'?'disabled':''}>${done&&x.id==='daily'?t('done'):t('start')}</button></article>`).join('')}
