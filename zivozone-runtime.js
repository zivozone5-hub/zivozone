...
(() => {
  ...
  // V1059 — Forensic Lab expansion to a complete 20-question case.
  bank.forensic.questions.push(
    Q('forensic-11','choice',O('أثر حذاء قرب النافذة يطابق مقاسًا شائعًا. ما قيمة هذا الدليل؟','A shoeprint near the window matches a common size. What is its value?','窗边鞋印符合常见尺码。它的价值是什么？','खिड़की के पास जूते का निशान आम आकार से मेल खाता है। इसका मूल्य क्या है?','Una huella cerca de la ventana coincide con una talla común. ¿Qué valor tiene?'),[O('قرينة تحتاج ربطها بعناصر أخرى','A clue that needs correlation with other evidence'),O('إثبات هوية الشخص وحده','Proof of identity by itself'),O('إثبات وقت الجريمة','Proof of the crime time'),O('دليل غير مفيد إطلاقًا','Completely useless evidence')],0,5,{image:'assets/forensic/scene-door.svg'}),
    Q('forensic-12','choice',O('الهاتف وُجد والشاشة مضاءة، لكن البطارية 2%. ما الخطوة الأدق؟','A phone is found with the screen on but 2% battery. What is the most precise next step?','发现手机屏幕亮着但电量2%。最准确的下一步是什么？','फोन स्क्रीन चालू है लेकिन बैटरी 2% है। सबसे सटीक अगला कदम क्या है?','Se encuentra un teléfono con pantalla encendida y 2% de batería. ¿Siguiente paso?'),[O('توثيق حالته وفحص السجلات قبل فقدان البيانات','Document its state and preserve logs before data is lost'),O('إطفاؤه فورًا','Turn it off immediately'),O('حذف الرسائل','Delete the messages'),O('تخمين وقت الاستخدام','Guess when it was used')],0,5,{image:'assets/forensic/scene-desk.svg'}),
    Q('forensic-13','choice',O('انعكاس في مرآة يظهر جسمًا في زاوية لا تظهر مباشرة. ما الأفضل؟','A mirror reflection shows an object not directly visible. What is best?','镜中反射显示一个直接看不到的物体。最佳做法是什么？','एक दर्पण प्रतिबिंब में सीधा न दिखने वाली वस्तु दिखती है। सर्वोत्तम कदम?','Un reflejo muestra un objeto que no se ve directamente. ¿Qué hacer?'),[O('توثيق زاوية الانعكاس ومقارنتها بالمشهد','Document the reflection angle and compare it with the scene'),O('اعتبار الصورة دليلًا مطلقًا','Treat the image as absolute proof'),O('تجاهل المرآة','Ignore the mirror'),O('تغيير مكان المرآة','Move the mirror')],0,6,{image:'assets/forensic/scene-camera.svg'}),
    Q('forensic-14','choice',O('خريطة صغيرة عليها سهم أحمر بلا تاريخ. ماذا تستنتج؟','A small map has a red arrow but no date. What can you infer?','一张小地图有红色箭头但没有日期。你能推断什么？','एक छोटे नक्शे पर लाल तीर है लेकिन तारीख नहीं। क्या निष्कर्ष?','Un mapa pequeño tiene una flecha roja pero no fecha. ¿Qué puedes inferir?'),[O('وجود اتجاه مقصود لكن السياق ما زال مجهولًا','There is an intended direction, but context remains unknown'),O('أنه يحدد مكان الجريمة قطعًا','It definitely marks the crime location'),O('أنه حديث العهد حتمًا','It is definitely recent'),O('أنه من الجاني','It is definitely from the perpetrator')],0,6,{image:'assets/forensic/scene-desk.svg'}),
    Q('forensic-15','choice',O('ترتيب ثلاثة أشياء تغيّر بين صورتين للمشهد. ما أول ما تقارنه؟','Three objects changed position between two scene photos. What do you compare first?','两张现场照片中三个物体位置发生变化。首先比较什么？','दो वस्तुओं की स्थिति दो तस्वीरों में बदली। पहले क्या तुलना करेंगे?','Tres objetos cambiaron de posición entre dos fotos. ¿Qué comparas primero?'),[O('الوقت وزاوية التصوير وتسلسل الصور','Timestamps, camera angle, and image sequence'),O('من يبدو مشبوهًا','Who looks suspicious'),O('أكثر صورة درامية','The more dramatic photo'),O('لون الجدران فقط','Only wall color')],0,7,{image:'assets/forensic/scene-camera.svg'}),
    Q('forensic-16','choice',O('ظهر ظل طويل في لقطة واحدة فقط. ما التفسير الأول الأقل افتراضًا؟','A long shadow appears in only one frame. What is the least-assumptive first explanation?','一个长影子只出现在一帧。最少假设的首个解释是什么？','एक लंबी छाया केवल एक फ्रेम में दिखती है। सबसे कम अनुमान वाला पहला स्पष्टीकरण?','Una sombra larga aparece en un solo fotograma. ¿Primera explicación con menos supuestos?'),[O('تغيّر مصدر الضوء أو زاوية الحركة','A change in light source or movement angle'),O('شخص مختبئ بالتأكيد','A person is definitely hiding'),O('الكاميرا تكذب دائمًا','The camera always lies'),O('الظل دليل هوية','The shadow proves identity')],0,7,{image:'assets/forensic/scene-camera.svg'}),
    Q('forensic-17','choice',O('وجدت حقيبة مغلقة وبداخلها ورقة باسم شخص. ما الذي لا يمكنك إثباته مباشرة؟','You find a closed bag containing a note with someone’s name. What can you not prove directly?','发现一个关闭的包，里面有写着某人名字的纸条。不能直接证明什么？','एक बंद बैग में किसी के नाम वाला नोट है। सीधे क्या साबित नहीं होता?','Encuentras una bolsa cerrada con una nota con un nombre. ¿Qué no puedes probar directamente?'),[O('أن صاحب الاسم هو مالك الحقيبة أو كاتب الورقة','That the named person owns the bag or wrote the note'),O('وجود الاسم على الورقة','That the name is on the note'),O('وجود الحقيبة','That the bag exists'),O('أن الورقة جزء من الدليل','That the note is evidence to examine')],0,8,{image:'assets/forensic/scene-desk.svg'}),
    Q('forensic-18','choice',O('تسجيل قصير يظهر بابًا يفتح ثم ينغلق. ما أهم شيء قبل الاستنتاج؟','A short recording shows a door opening and closing. What matters most before inference?','短视频显示门打开又关闭。推断前最重要的是什么？','एक छोटी रिकॉर्डिंग दरवाज़ा खुलते और बंद होते दिखाती है। निष्कर्ष से पहले सबसे महत्वपूर्ण क्या है?','Una grabación muestra una puerta abriéndose y cerrándose. ¿Qué importa antes de inferir?'),[O('فحص كامل التسلسل ووقت التسجيل ومصدر الكاميرا','Check the full sequence, timestamp, and camera source'),O('اعتبار الفتح دليل دخول','Assume opening proves entry'),O('تجاهل وقت التسجيل','Ignore the recording time'),O('حذف اللقطة','Delete the clip')],0,8,{image:'assets/forensic/scene-camera.svg'}),
    Q('forensic-19','choice',O('لديك دليلان متعارضان، لكن أحدهما موثق ومؤرخ والآخر شهادة ذاكرة بعد أيام. أيهما تبدأ به؟','Two pieces of evidence conflict; one is documented and timestamped, the other is a memory-based statement days later. Which do you start with?','两份证据冲突：一份有记录和时间戳，另一份是几天后的记忆陈述。先从哪份开始？','दो साक्ष्य विरोधी हैं; एक दस्तावेज़ित और समय-मुद्रित है, दूसरा दिनों बाद की स्मृति है। किससे शुरू करेंगे?','Dos pruebas chocan: una está documentada y fechada, otra es un recuerdo días después. ¿Con cuál empiezas?'),[O('أبدأ بالأدلة الموثقة ثم أختبر الشهادة مقابلها','Start with documented evidence, then test the statement against it'),O('أرفض الشهادة نهائيًا','Reject the statement outright'),O('أصدق الذاكرة دائمًا','Always trust memory'),O('أختار الرواية الأكثر إثارة','Choose the most dramatic story')],0,9,{image:'assets/forensic/scene-desk.svg'}),
    Q('forensic-20','choice',O('في النهاية، لديك رواية تفسر الأدلة العشرين دون قفزات منطقية كبيرة. ما القرار الصحيح؟','At the end, one narrative explains the evidence without major logical leaps. What is the correct decision?','最后，一个故事解释证据且没有重大逻辑跳跃。正确决定是什么？','अंत में एक कथा सभी साक्ष्यों को बड़े तार्किक छलांग के बिना समझाती है। सही निर्णय?','Al final, una historia explica las pruebas sin grandes saltos lógicos. ¿Decisión correcta?'),[O('اعتمادها كأفضل فرضية حالية مع إبقاء باب التحقق مفتوحًا','Use it as the strongest current hypothesis while keeping verification open'),O('اعتبارها حقيقة نهائية لا تقبل المراجعة','Treat it as final unquestionable truth'),O('تجاهل الأدلة المخالفة','Ignore conflicting evidence'),O('اختيار نظرية عشوائية','Choose a random theory')],0,10,{image:'assets/forensic/scene-camera.svg'})
  );
...
(() => {
  ...
  function guestGate(id){clearQuestionTimer();openModal(`<button class="modal-close" data-close>×</button><span class="eyebrow">ZIVOZONE · MEMBERS</span><h2>هذا التحدي متاح للأعضاء المسجلين فقط</h2><p>لقد أنهيت السؤال الأول. أنشئ حسابًا مجانيًا أو سجّل الدخول لمتابعة التحدي وحفظ نتيجتك.</p><div class="modal-actions"><button class="btn btn-primary" id="create-now">إنشاء حساب</button><button class="btn btn-ghost" id="login-now">تسجيل الدخول</button><button class="btn btn-ghost" data-action="return-challenges">العودة</button></div></div>`,'zivo-member-gate');$('#create-now').onclick=()=>authModal(()=>{closeModal();game.guest=false;nextQuestion(350)});$('#login-now').onclick=()=>{closeModal();let mode='login';authModal(()=>{closeModal();game.guest=false;nextQuestion(350)})}}
  ...
  function identity(){
    ...
    const paragraphs={
      ar:['أنت شخص تحليلي، تراقب التفاصيل قبل القرار، وتبحث عن الأنماط الخفية بدل الاكتفاء بالانطباع الأول. تميل إلى جمع المعلومات، مقارنة الاحتمالات، ثم اختيار الطريق الذي يبدو أكثر اتساقًا. قوتك في الهدوء والتركيز وتحويل التعقيد إلى خطوات قابلة للفهم. عندما تواجه مشكلة جديدة، غالبًا تبحث عن القاعدة التي تحكمها قبل أن تتحرك. قد تحتاج أحيانًا إلى منح نفسك مساحة أكبر للتجربة بدل انتظار الصورة الكاملة. في ZIVOZONE يناسبك تحدي المختبر الجنائي أو مختبر الذكاء، لأنهما يكافئان الملاحظة وربط الأدلة واتخاذ القرار على أساس منطقي. هدفك القادم: ثق بتحليلك، لكن لا تخف من اختبار فرضيتك بسرعة.','أنت حاسم وطموح، تميل للمبادرة وتحمل الضغط، وتفضّل القرار الواضح على الانتظار الطويل. عندما يظهر تحدٍ جديد، غالبًا تبحث عن نقطة البداية ثم تتحرك، حتى لو لم تكن كل التفاصيل مكتملة. قوتك في الجرأة وتحمل المسؤولية وتحويل المواقف الصعبة إلى طاقة للحركة. قد تحتاج أحيانًا إلى إبطاء الإيقاع عندما يكون القرار معقدًا حتى لا تتحول السرعة إلى استعجال. في ZIVOZONE يناسبك تحدي الغرفة المظلمة أو التحديات الرياضية، لأنك تستمتع بالضغط والاختيار والمواجهة. أفضل نسخة منك تجمع بين شجاعتك الحالية ولمسة إضافية من التحليل قبل الحسم.','أنت شخص اجتماعي وداعم، ترى الناس جزءًا أساسيًا من النجاح ولا تحب أن تعمل بمعزل عن الآخرين عندما يكون التعاون قادرًا على تحسين النتيجة. تميل إلى الاستماع، جمع وجهات النظر، وبناء الثقة قبل اتخاذ القرار. قوتك في التواصل وقراءة احتياجات المجموعة والمحافظة على التماسك عندما يرتفع الضغط. أحيانًا قد تمنح آراء الآخرين وزنًا أكبر من اللازم، لذلك من المفيد أن تحافظ على رأيك المستقل أيضًا. في ZIVOZONE يناسبك تحدي الاستراتيجية أو تحديات كرة القدم التي تعتمد على القرار والتعاون. كلما وازنت بين دعم الفريق وثقتك بقرارك، أصبحت أكثر تأثيرًا.','أنت مستكشف ومبدع، يثيرك المجهول وتشدك الأسئلة التي لا تملك لها إجابة جاهزة. تكره القوالب الثابتة وتميل إلى تجربة طرق غير مألوفة، وربط أفكار بعيدة عن بعضها لصناعة احتمال جديد. قوتك في الفضول والخيال والاستعداد لتغيير الخطة عندما تظهر معلومة جديدة. التحدي لديك هو تحويل كثرة الأفكار إلى خطوات عملية حتى لا يبقى الإبداع في مرحلة الاحتمالات فقط. في ZIVOZONE يناسبك تحدي الغرفة المظلمة أو مختبر المعرفة، لأنهما يمنحانك مساحة للفضول والانتباه والتفكير المختلف. جرّب أفكارك، ثم اختبرها بدل الاكتفاء بتخيلها.'],
    ...
    const step=()=>{
      ...
      $$('.id-choice').forEach(b=>b.onclick=()=>{clearIdentityTimer();if(window.__zivoIdentityTimeout){window.__zivoIdentityTimeout=false;i++;step();return}s[Number(b.dataset.n)]++;i++;if(i===1&&!A.isLoggedIn()){openModal(`<div class="zivo-member-gate"><span class="eyebrow">ZIVOZONE · MEMBERS</span><h2>أكمل اختبار من أنا بحسابك</h2><p>تمت تجربة السؤال الأول. سجّل الدخول أو أنشئ حسابًا مجانيًا لمتابعة الأسئلة وحفظ التحليل النهائي.</p><div class="modal-actions"><button class="btn btn-primary" id="identity-register">إنشاء حساب</button><button class="btn btn-ghost" id="identity-login">تسجيل الدخول</button><button class="btn btn-ghost" data-action="return-challenges">العودة</button></div></div>`,'zivo-member-gate');$('#identity-register').onclick=()=>authModal(()=>{closeModal();step()});$('#identity-login').onclick=()=>{closeModal();authModal(()=>{closeModal();step()})};return}step()});
      startIdentityTimer();
    };step();
  }
  function renderChallenges(){const today=new Date().toISOString().slice(0,10),done=localStorage.getItem('zivo_daily_'+today);const raw=C.getAll();const seenIds=new Set();const list=raw.filter(x=>x&&x.id&&!seenIds.has(x.id)&&seenIds.add(x.id));const visual={iq:'assets/challenges/iq.svg',science:'assets/challenges/science.svg',daily:'assets/challenges/daily.svg',football:'assets/challenges/football.svg',logic:'assets/challenges/logic.svg',memory:'assets/challenges/memory.svg',math:'assets/challenges/math.svg',forensic:'assets/challenges/forensic.svg',horror:'assets/challenges/horror.svg',strategy:'assets/challenges/strategy.svg'};const cardVisual=x=>visual[x.id]?`<img class="challenge-visual" src="${visual[x.id]}" alt="${esc(loc(x.title))}" loading="lazy">`:`<div class="challenge-visual challenge-visual-fallback" aria-hidden="true">${esc(x.icon||'🎯')}</div>`;$('#challenge-list').innerHTML=list.map(x=>{const locked=!!(done&&x.id==='daily');return `<article class="challenge-card${x.id==='horror'?' special':''}" data-challenge="${esc(x.id)}" tabindex="0" role="button" aria-disabled="${locked}" aria-label="${esc(loc(x.title))} — ${locked?t('done'):t('start')}">${cardVisual(x)}<div class="challenge-copy"><span class="card-tag${x.id==='horror'?' danger':''}">${(x.id==='forensic'||x.id==='horror')?'20 سؤال':'10 أسئلة'} · 30s</span><h3>${esc(loc(x.title))}</h3><p>${esc(loc(x.desc||'تحدٍ تفاعلي مصمم لقياس مهارة محددة.'))}</p></div><button class="z58-hidden-launcher" type="button" data-challenge="${esc(x.id)}" ${locked?'disabled':''} tabindex="-1" aria-hidden="true">${locked?t('done'):t('start')}</button></article>`;}).join('');}
  ...
  async function askAI(message){const payload={message:String(message||'').slice(0,1000),language:lang(),player:{level:state.level,xp:state.xp,gamesPlayed:state.gamesPlayed,coins:state.coins}};try{const f=window.firebase?.functions?.();if(f){const fn=f.httpsCallable(API.aiCallable||'zivoAI');const r=await fn(payload);if(r?.data?.reply)return r.data.reply}}catch(e){console.warn('ZIVO AI callable:',e)}if(API.aiEndpoint){try{const r=await fetch(API.aiEndpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});if(r.ok){const d=await r.json();if(d.reply)return d.reply}}catch(e){console.warn('ZIVO AI endpoint:',e)}}return localAIReply(message)}
